...
ALTER TABLE public.orders DISABLE TRIGGER trg_notify_orders_status;
ALTER TABLE public.orders DISABLE TRIGGER trg_order_completed_email;
ALTER TABLE public.orders DISABLE TRIGGER trg_orders_oos_email;

-- 2) Create the standardized jsonb-only wrapper
...
-- 3) Update functions to use the jsonb-only wrapper (remove ::text casts)
...
-- trgfn_orders_oos_email: Remove timeout_milliseconds and use standardized call
...
-- 4) Re-enable the notification/email triggers on public.orders
