-- PLANT AND STORE NORMALIZATION - CLEAN APPROACH
...
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON ot_platform_users;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON platform_users;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON orders;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON managers;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON mto_orders;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON wheel_orders;
DROP TRIGGER IF EXISTS validate_plant_and_store_trigger ON warranty_orders;

-- Drop conflicting functions
DROP FUNCTION IF EXISTS public.validate_plant_name();
DROP FUNCTION IF EXISTS public.validate_plant_and_store_format();

-- Step 2: Fix Legacy Store Values
...
-- Step 3: Fix Legacy Plant Values
...
-- Step 6: Apply store normalization
...
-- Step 7: Final validation query
SELECT 'VALIDATION_COMPLETE' as status, 
       'Plant and store normalization migration completed successfully' as message
UNION ALL
SELECT 'NEXT_STEPS' as status,
       'All legacy store/plant formats have been normalized to 3-digit format' as message;