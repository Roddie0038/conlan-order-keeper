-- COMPREHENSIVE PLANT AND STORE NORMALIZATION MIGRATION - CORRECTED
...
-- Phase 3: Enhanced Plant Normalization Function (replace existing)
...
-- Phase 6: Drop existing conflicting triggers
...
-- Phase 7: Replace the validation trigger function
...
AS $function$
BEGIN
    -- Handle different tables with their specific column names
    IF TG_TABLE_NAME = 'ot_platform_users' THEN
    ...
    ELSIF TG_TABLE_NAME IN ('mto_orders', 'wheel_orders', 'warranty_orders') THEN
        IF NEW.store IS NOT NULL THEN
            NEW.store := normalize_store_format(NEW.store);
        END IF;
    END IF;
...
-- Phase 8: Create comprehensive validation triggers
