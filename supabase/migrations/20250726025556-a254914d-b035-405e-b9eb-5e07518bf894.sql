-- Phase 1: Database Schema and Data Migration (Handle NULL stores)

-- First, make store_number nullable temporarily
ALTER TABLE public.ordering_email_recipients 
ALTER COLUMN store_number DROP NOT NULL;

-- Add missing columns to ordering_email_recipients table
ALTER TABLE public.ordering_email_recipients 
...
-- Split into store-based and plant-based users
...
SELECT DISTINCT
  CASE 
    ...
    ELSE COALESCE(plant, 'PLANT_USER') -- Use plant or placeholder for plant-based users
  END as store_number,
  CASE 
