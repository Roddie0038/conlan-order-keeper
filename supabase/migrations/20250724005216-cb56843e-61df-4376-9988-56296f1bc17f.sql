-- COMPREHENSIVE STORE NORMALIZATION MIGRATION (Both Platforms) - FINAL FIX
-- This migration implements store format standardization across all tables

-- First, drop dependent views to avoid conflicts
DROP VIEW IF EXISTS public.store_normalization_summary CASCADE;
DROP VIEW IF EXISTS public.store_normalization_verification CASCADE;
...
-- Apply triggers to all relevant tables (only if tables exist)
DO $$
BEGIN
    -- Orders table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'orders') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON orders FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- MTO Orders table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mto_orders') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON mto_orders FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Wheel Orders table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'wheel_orders') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON wheel_orders FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Warranty Orders table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'warranty_orders') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON warranty_orders FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- OT Platform Users table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ot_platform_users') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON ot_platform_users FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Platform Users table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'platform_users') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON platform_users FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Managers table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'managers') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON managers FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Ordering Email Logs table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ordering_email_logs') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON ordering_email_logs FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Ordering Email Recipients table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'ordering_email_recipients') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON ordering_email_recipients FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
    
    -- Store Email Recipients table
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'store_email_recipients') THEN
        EXECUTE 'CREATE TRIGGER normalize_store_trigger BEFORE INSERT OR UPDATE ON store_email_recipients FOR EACH ROW EXECUTE FUNCTION trigger_normalize_store_format()';
    END IF;
END $$;

-- ===== PHASE 3: Historical Data Cleanup =====

-- Update existing data in all tables
...
-- Update Store Email Recipients if it exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'store_email_recipients') THEN
...
-- ===== PHASE 4: Update has_store_access function =====
...
-- ===== PHASE 5: Create New Validation View =====
...
SELECT 'Store normalization migration completed successfully across both platforms!' as status;