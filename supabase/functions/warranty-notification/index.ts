...
serve(async (req) => {
  ...
  try {
    ...
    const body = {
      ...
      store_number: payload.store_number ?? (payload as any).store,
      plant_id: payload.plant_id ?? (payload as any).plant,
      metadata: payload,
      options: { dry_run: !!payload.dry_run, enforce_scopes: true, source: "warranty-notification" },
    };
