...
export default function Dashboard() {
  ...
  const handleSearch = () => {
    ...
    setIsSearching(true);
    const results = inventory.filter(item => item.product_number.toLowerCase().includes(searchQuery.toLowerCase()) || item.description.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 5);
  ...
  return <div className="min-h-screen bg-slate-900 text-white p-6">
      <div className="container mx-auto">
        <header className="flex justify-between items-center mb-8">
          ...
          <Button variant="outline" onClick={handleLogout} className="border-orange-500 text-white font-bold hover:bg-orange-500">
            Logout
          </Button>
        </header>
          ...
          <Card className="bg-gradient-to-br from-blue-800 to-indigo-900 border-2 border-blue-400 shadow-lg">
            <CardContent className="p-6">
              ...
              <div className="flex flex-col space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  <div className="md:col-span-3">
                    ...
                    <div className="flex gap-2">
                      <Input id="search" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="Enter search term..." className="bg-white/20 border-white/30 text-white placeholder:text-gray-300" />
                  ...
                  <div className="md:col-span-1 flex items-end">
                    <Button onClick={() => navigate('/relentless-inventory')} className="w-full bg-blue-600 hover:bg-blue-700">
                      View All Inventory
                    </Button>
                  </div>
                </div>

                {searchResults.length > 0 && <div className="mt-4 bg-white/10 rounded-lg p-4">
                    <h3 className="text-lg font-semibold text-white mb-2">Results</h3>
                    <div className="space-y-2 max-h-60 overflow-y-auto">
                      {searchResults.map(item => <div key={item.id} className="p-3 bg-white/20 rounded-lg">
                ...
                {searchQuery && searchResults.length === 0 && !isSearching && <div className="bg-white/10 rounded-lg p-4 text-center">
                    <p className="text-white">No results found for "{searchQuery}"</p>
                  </div>}

                {loading && <div className="bg-white/10 rounded-lg p-4 text-center">
                ...
                <div className="group perspective-1000">
                  ...
                  `}>
                    <div className={`
                      ...
                      ${item.title === "WAREHOUSE INVENTORY" ? "border-4 border-purple-400 shadow-[0_10px_30px_rgba(138,43,226,0.3)]" : "shadow-[8px_8px_0px_0px_rgba(0,0,0,0.3)] group-hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,0.25)]"}
