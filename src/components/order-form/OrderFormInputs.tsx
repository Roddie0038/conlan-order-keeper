...
import { scheduleOptions, crossDockOptions, stores, type FormData, storeManagerEmails, priorityOptions } from "./formConfig";
...
export const OrderFormInputs = ({
  ...
  return <form onSubmit={onSubmit} className="space-y-6 max-w-2xl mx-auto p-6 shadow rounded-3xl bg-gray-500">
      <div className="space-y-4">
        <FormField label="Priority" required value={formData.priority} onChange={value => onChange("priority", value)} options={priorityOptions} placeholder="Select priority level" />
        ...
        <FormField label="Manager's Email" type="email" value={formData.managersEmail || ''} onChange={() => {}} disabled={true} placeholder="Manager's email will be automatically set" />
      ...
      <Button type="submit" className="w-full text-slate-50 rounded-3xl bg-rose-600 hover:bg-rose-500">
        ADD TO ORDER
      </Button>
    </form>;
};