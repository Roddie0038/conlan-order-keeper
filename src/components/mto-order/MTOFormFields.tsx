          ...
          {casingGrades.map(grade => (
            <div key={grade.value} className="flex items-center space-x-2">
              <Checkbox 
                ...
                className="font-extrabold text-7xl text-center rounded-full text-red-600 bg-slate-500 hover:bg-slate-400"
              />
              <Label 
                htmlFor={grade.value} 
                className="px-0 rounded-full bg-green-400 hover:bg-green-300"
              >
                {grade.name}
              </Label>
            </div>
