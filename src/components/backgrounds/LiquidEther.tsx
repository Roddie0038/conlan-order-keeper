  ...
  useEffect(() => {
    ...
    class AutoDriver {
      ...
      update() {
        ...
        if (dist < 0.01) return this.pickNewTarget();
        dir.normalize();
        let ramp = 1;
...
`;
    ...
    class ShaderPass {
      props: any; scene: THREE.Scene | null = null; camera: THREE.Camera | null = null;
      material: THREE.RawShaderMaterial | null = null; geometry: THREE.PlaneGeometry | null = null;
      plane: THREE.Mesh | null = null; uniforms: any;
    ...
    class WebGLManager {
      props: any; output!: Output; _loop!: () => void; _resize!: () => void; _onVisibility!: () => void;
      running = false; lastUserInteraction = performance.now();
  ...
  // Respect prefers-reduced-motion: hide animation
  useEffect(() => {
    const media = window.matchMedia('(prefers-reduced-motion: reduce)');
    if (!mountRef.current) return;
    if (media.matches) mountRef.current.style.display = 'none';
    const onChange = () => {
      if (!mountRef.current) return;
      mountRef.current.style.display = media.matches ? 'none' : '';
    };
